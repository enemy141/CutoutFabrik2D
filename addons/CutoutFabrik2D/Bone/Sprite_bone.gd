tool
extends Sprite
class_name SpriteBone,"res://addons/CutoutFabrik2D/Bone/Spritebone.png"
...
var enable_scale : bool = false
var const_scale : Vector2 = Vector2(1,1)
...
func _set(property, value):
	...
	elif property == "BONE/Constraint_Scale":
		enable_scale = value
		property_list_changed_notify()
		return true
		
	elif property == "BONE/Scale":
		const_scale = value
		property_list_changed_notify()
		return true
...
func _get_property_list():
	...
	dict = {'name' : "BONE/Auto_angle",
			...
			'usage' : PROPERTY_USAGE_DEFAULT}
	list.append(dict)
	if auto_length_cal != true:
		dict = {'name' : "BONE/Length",
				'type' : TYPE_REAL,
				'hint' : PROPERTY_HINT_NONE,
				'usage' : PROPERTY_USAGE_DEFAULT}
		list.append(dict)
	if auto_angle_cal != true:
	...
	dict = {'name' : "BONE/LineColor",
			...
			'usage' : PROPERTY_USAGE_DEFAULT}
	list.append(dict)
	dict = {'name' : "BONE/Constraint_Scale",
			'type' : TYPE_BOOL,
			'hint' : PROPERTY_HINT_NONE,
			'usage' : PROPERTY_USAGE_DEFAULT}
	list.append(dict)
	if enable_scale == true:
		dict = {'name' : "BONE/Scale",
			'type' : TYPE_VECTOR2,
			'hint' : PROPERTY_HINT_NONE,
			'usage' : PROPERTY_USAGE_DEFAULT}
		list.append(dict)
